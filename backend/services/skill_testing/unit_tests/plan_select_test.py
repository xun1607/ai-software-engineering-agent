...
async def test_nodes_workflow():
    ...
    responses = {
        "Lead Software Architect": json.dumps({"plan": ["Analyze the Java crash log", "Read the source code"]}),
        "Tool Selector": json.dumps({"thought": "The task is about logs, choosing analyzer", "selected_skill": "analyze-stacktrace"})
    }
    ...
    print("\n--- Testing Plan Node ---")
    state = await plan_node(state, mock_llm)
    ...
    state = await select_skill_node(state, mock_llm)
...
if __name__ == "__main__":
    asyncio.run(test_nodes_workflow())