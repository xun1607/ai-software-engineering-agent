...
acceptance_criteria:
  - "output.fix_suggestion must have at least 30 characters explaining the root cause"
  - "output.code_snippet must contain Python keywords (def, if, return, self, None, assert, raise, or try)"
  - "output.fix_suggestion must reference the error_type or variable from input"
...
fallback_strategy: "If the error type is unknown, provide a generic defensive programming suggestion wrapping the problematic line in a try/except block."
